...
class BusinessAccounts extends Component {
    ...
    renderBusinessAccounts = () => {
        var filtered = this.state.business.filter(each => each.name && each.name.toLowerCase().includes(this.state.keyword.toLowerCase()) && each.status === 'approved');
        return (
            <Container>
                {
                                ...
                                <Row>
                                    <Col className="col-10 d-flex align-items-center" onClick={() => {
                                        ...
                                        <CardImg className="img-fluid rounded-circle"
                                                 src={each.icon ? each.icon : each.img ? each.img : defaultLogoImg}
                                                 alt="No Image"
                                                 style={{width: 40, height: 40, border: '2px solid #f7d907'}}/>
                                        <CardText className="mx-3">{each.name}</CardText>
                                    </Col>
                                    <Col className="col-2 d-flex align-items-center px-0">
                                        <Button className="rounded d-flex justify-content-center align-items-center"
                                                color="#f7d907" style={{backgroundColor: "#f7d907"}}
                                                onClick={() => this.props.history.push('/admin/businessaccounts/view', {bid: each.id})}>
                                            <span>VIEW</span>
                                        </Button>
                                        {
                                            each.active &&
                                            <Button
                                                className="rounded d-flex justify-content-center align-items-center ml-3"
                                                color="e03930" style={{backgroundColor: "#e03930", width: 105}}
                                                onClick={() => this.onDeactivate(each)}>
                                        ...
                                        }
                                        {
                                            ...
                                            <Button
                                                className="rounded d-flex justify-content-center align-items-center ml-3"
                                                color="0db51a" style={{backgroundColor: "#0db51a", width: 105}}
                                                onClick={() => this.onActivate(each)}>
                ...
                <div className="d-flex justify-content-end mb-3"
                     style={{position: 'fixed', top: 15, right: 15, zIndex: 1100}}>
                    <div className="d-flex align-items-center">
                        ...
                        <div className="form-group row ml-1 mb-0">
                            <div className="col-md-3">
                                <input className="form-control" type="text" defaultValue='' placeholder='Search'
                                       style={{width: 200}} onChange={(e) => this.setState({keyword: e.target.value})}/>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <Row>
                    <Col className="col-3">
                        <CardBody className="bg-white">
                            <CardImg className="img-fluid" src={business.img} alt="No Image"
                                     style={{borderRadius: 10}}/>
                            <CardTitle className="mt-3">{business.name}</CardTitle>
                            <CardText className="d-flex justify-content-between">{membership?.level}<i
                                className="text-success">${membership?.price}</i></CardText>
                            <CardText><i className={`${Icons.location}`}></i><span
                                className="ml-2">{business.address}</span></CardText>
                            ...
                            <CardText><i className={`${Icons.timer}`}></i><span
                                className="ml-2">{business.operatingHours.from} - {business.operatingHours.to}</span></CardText>
                        </CardBody>
                    </Col>
                    <Col className="col-9">
                        {
                                ...
                                return (
                                    <div key={index} className="mb-3">
                                        <CardBody className="bg-white w-75">
                                            <div
                                                className="modal-header border-0 d-flex align-items-center bg-white p-0">
                                                <div className="d-flex align-items-center">
                                                    <div className="mx-2">
                                                        <span className="text-info font-size-16">{each.name}</span>
                                                        <br/>
                                                        <span className="font-size-12"><i className={`${Icons.location} mr-2`}/>{each.address}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="modal-body p-0 bg-white mt-3">
                                                <CardImg className="img-fluid" src={each.img ? each.img : noImg}
                                                         alt="No Image" style={{height: 300}}/>
                                                <CardTitle
                                                    className="d-flex justify-content-between align-items-center mt-3">
                                                    <span
                                                        className="text-success font-size-18"><b>${each.price}</b></span>
                                                    <span
                                                        className="">{each.days} days, {each.hunters} {each.hunters > 1 ? 'hunters' : 'hunter'}</span>
                                                </CardTitle>
                                                <CardText>
                                                ...
                                                <CardTitle
                                                    ...
                                                    <div className="d-flex justify-content-end align-items-center"
                                                        ...
                                                        />
                                                        <span className="mt-3"
                                                              style={{marginLeft: -10}}>{(each.rating?.toFixed(1)) ?? 0}</span>
                                                    </div>
                                                </CardTitle>
                                            </div>
                                        </CardBody>
            ...
            <Container>
                {
                    this.props.data.business.map((each, index) => {
                        ...
                        return (
                            <CardBody key={index} className="bg-white" style={{borderBottom: '2px solid #eee'}}>
                                <Row
                                    onClick={() => this.props.history.push('/admin/businessaccounts/requests/detail', {business: each})}
                                    style={{cursor: "pointer"}}>
                                    <Col className="col-10 d-flex align-items-center">
                                        <CardImg className="img-fluid rounded" src={each.img ? each.img : noImg}
                                                 alt="No Image" style={{width: 200, height: 130}}/>
                                        <CardBody>
                                            <span className="font-size-16"><b>{each.name}</b></span>
                                            <div className="d-flex align-items-center"><i
                                                className={`${Icons.location}`}></i><span
                                                className="ml-2">{each.address}</span></div>
                                            <div className="d-flex align-items-center"><i
                                                className={`${Icons.domain}`}></i><span
                                                className="ml-2">{each.site}</span></div>
                                            <div className="d-flex align-items-center"><i
                                                className={`${Icons.email}`}></i><span
                                                className="ml-2">{each.email}</span></div>
                                        </CardBody>
                                    </Col>
                                    <Col className="col-2 d-flex align-items-center px-0">
                                        <i>Request Date:</i>
            ...
            <Container>
                <CardBody>
                    <div className="form-group row">
                        <label htmlFor="example-text-input" className="col-md-2 col-form-label text-right">Business
                            Logo</label>
                        <div className="col-md-4">
                            <CardImg className="img-fluid" src={business.img ? business.img : noImg} alt="No Image"
                                     style={{borderRadius: 10}}/>
                        </div>
                    </div>
                    <div className="form-group row">
                    ...
                    <div className="form-group row">
                        <label htmlFor="example-text-input"
                               className="col-md-2 col-form-label text-right">Address</label>
                        <div className="col-md-6">
                    ...
                    <div className="form-group row">
                        <label htmlFor="example-tel-input" className="col-md-2 col-form-label text-right">Contact
                            number</label>
                        <div className="col-md-6">
                    ...
                    <div className="form-group row">
                        <label htmlFor="example-email-input"
                               className="col-md-2 col-form-label text-right">Email</label>
                        <div className="col-md-6">
                    ...
                    <div className="form-group row">
                        <label htmlFor="example-url-input"
                               className="col-md-2 col-form-label text-right">Website</label>
                        <div className="col-md-6">
                            <input className="form-control" type="url" disabled={true} defaultValue={business.site}/>
                        </div>
                    </div>
                    <div className="form-group row">
                        <label htmlFor="example-text-input" className="col-md-2 col-form-label text-right">Subscription
                            Plan</label>
                        <div className="col-md-6">
                            <input className="form-control" type="text" disabled={true}
                                   defaultValue={membership ? membership.level : ''}/>
                        </div>
                    </div>
                    <div className="form-group row">
                        <label htmlFor="example-text-input"
                               className="col-md-2 col-form-label text-right">Username</label>
                        <div className="col-md-6">
                            <input className="form-control" type="text" disabled={true} defaultValue={user?.name}/>
                        </div>
                    </div>
                    <div className="form-group row">
                        <label htmlFor="example-text-input" className="col-md-2 col-form-label text-right">Temporary
                            Password</label>
                        <div className="col-md-6">
                    ...
                    <div className="form-group row">
                        <label htmlFor="example-time-input" className="col-md-2 col-form-label text-right">Operating
                            Hours</label>
                        <div className="col-md-6 d-flex px-0">
                            <div className="col-md-6">
                                <input className="form-control" type="text" disabled={true}
                                       defaultValue={business.operatingHours.from} id="operating-hour-input-from"/>
                            </div>
                            <div className="col-md-6">
                                <input className="form-control" type="text" disabled={true}
                                       defaultValue={business.operatingHours.to} id="operating-hour-input-to"/>
                            </div>
                        </div>
                    </div>
                    <div className="form-group row">
                        <label htmlFor="example-tel-input"
                               className="col-md-2 col-form-label text-right">Information</label>
                        <div className="col-md-6">
                            <textarea className="form-control" id="information" rows="10" disabled={true}
                                      defaultValue={business.desc}></textarea>
                        </div>
                    </div>
                    ...
                    <div className="d-flex justify-content-end mb-3"
                        ...
                        <div className="d-flex align-items-center">
                            <Button
                                ...
                                style={{backgroundColor: '#f7d907'}}
                                onClick={() => {
                                    this.onApproveRequest(business)
                                }}
                            >
                                <b className="text-dark">
                                    {
                                        (this.state.operation === 'approve account' && this.props.data.loading) &&
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    this.onDeleteRequest(business)
                                }}
                            >
                                <b className="text-white">
                                    {
                                        (this.state.operation === 'delete request' && this.props.data.loading) &&
                                        <i className="bx bx-loader bx-spin font-size-18 align-middle mr-2"></i>
                                    }
                                    DELETE REQUEST</b>
                            </Button>
